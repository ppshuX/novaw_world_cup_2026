...
2. 在 `src/data/matches.ts` 更新比赛日期、北京时间、球队、城市、球场和状态。
3. 若数据已经确认，把 `dataStatus` 改为 `official`。
4. 若数据还未确认，保持 `dataStatus: 'pending'`。
5. 若只是样式示例，保持 `dataStatus: 'mock'`。
6. 比赛结束后再填写 `homeScore`、`awayScore`，并把 `status` 改为 `已结束`。
7. 淘汰赛产生真实胜者后，再更新 `src/data/bracket.ts`。
...
V1 的目标是把赛程展示做好。未来可能加入轻量“看球记录 / 赛后备注”功能，例如记录某场比赛的高光时刻、补充中文备注或维护更新日志。但这不是 V1 核心，也不涉及金钱、竞猜平台、排行榜或用户社区。
...
这个项目最早来自一个个人暑期看球小站的想法，后来调整为面向中文用户的公开赛程展示网站。页面保留 NovaW 的一点个人风格，但主体验服务所有需要快速查看中文赛程的用户。
